...
public class WatsXpathDao {
  ...
  String sql= "UPDATE win_ta_script_metadata m SET m.xpath_location =:xpath WHERE m.script_number=:scripNumber and m.action=:action and m.line_number=:lineNumber and m.input_parameter like :params";
	...
	public Timestamp executionDate(String scripNumber) {
		Session session = entityManager.unwrap(Session.class);
		String sql="select CAST(EXECUTION_START_TIME AS DATE)  from WIN_TA_TEST_SET_LINES where script_number=:scripNumber and status='Pass' order by EXECUTION_START_TIME desc fetch first 1 rows only";
		SQLQuery query = session.createSQLQuery(sql);
		query.setParameter("scripNumber", scripNumber);
		List results = query.list();
...
//			  Timestamp ts=new Timestamp(obj);  
	...
	public Timestamp scriptUpdateDate(String scripNumber) {
		Session session = entityManager.unwrap(Session.class);
		String sql="select update_date from win_ta_script_metadata where script_number=:scripNumber order by update_date desc fetch first 1 rows only";
		SQLQuery query = session.createSQLQuery(sql);
		query.setParameter("scripNumber", scripNumber);
		List results = query.list();
		System.out.println("results1:;:"+results);
