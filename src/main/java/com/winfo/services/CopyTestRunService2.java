...
public class CopyTestRunService2 {
	...
	public int copyTestrun(@Valid CopytestrunVo copyTestrunvo) throws InterruptedException {
		...
		// Comparator<ScriptsData> scriptComparator = (ScriptsData s1,ScriptsData
		// s2)->s1.getSeqnum()-s2.getSeqnum();
...
//				System.out.println("id :" + copyTestrunDao.getIds());
			...
			Comparator<ScritplinesData> scriptLineComparator = (ScritplinesData s1,
					ScritplinesData s2) -> s1.getLine_number() - s2.getLine_number();
			...
			while (newScriptParamSeq < metadataList.size() && oldScriptParamSeq < scriptLineList.size()) {
				...
				if (setScriptlinedata.getInput_parameter() != null && getScriptlinedata.getInput_parameter() != null) {
					if (setScriptlinedata.getAction().equalsIgnoreCase(getScriptlinedata.getAction())
							&& setScriptlinedata.getInput_parameter()
									.equalsIgnoreCase(getScriptlinedata.getInput_parameter())
							&& setScriptlinedata.getLine_number() == getScriptlinedata.getLine_number()) {
...
//		setTestrundata.setScriptsdata(listsScriptdata);
	...
	private void addInputvalues(ScritplinesData getScriptlinedata, ScritplinesData setScriptlinedata,
			CopytestrunVo copyTestrunvo, ScriptsData setScriptdata) throws InterruptedException {
			...
			if ((setScriptlinedata.getDatatypes() != null && setScriptlinedata.getDatatypes() != "NA")
				...
				if (getInputvalues == null || "copynumber".equalsIgnoreCase(setScriptlinedata.getAction())) {
					...
					if (actionsList.contains(setScriptlinedata.getAction())) {
						...
						&& "copyTestRun".equalsIgnoreCase(copyTestrunvo.getRequesttype())) {
					setScriptlinedata.setInput_value(
